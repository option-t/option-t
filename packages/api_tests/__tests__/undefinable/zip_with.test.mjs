...
import { zipWithForUndefinable } from 'option-t/undefinable/zip_with';
...
for (const { a, b, expected } of SOME_VALUE_PATTERN) {
    test(`a=${String(a)}, b=${String(b)}`, (t) => {
        ...
        const actual = zipWithForUndefinable(a, b, (passedA, passedB) => {
...
for (const { a, b, expected } of NO_VALUE_PATTERN) {
    test(`a=${String(a)}, b=${String(b)}`, (t) => {
        ...
        const actual = zipWithForUndefinable(a, b, (_a, _b) => {
...
test(`should throw if the callback return ${NULL_VALUE_IN_THIS_TEST_CASE}`, (t) => {
    ...
    t.throws(
        () => {
            zipWithForUndefinable(a, b, (_a, _b) => {
